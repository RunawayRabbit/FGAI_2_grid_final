...
void AFGPlayer::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction(TEXT("SetStart"), IE_Pressed, this, &AFGPlayer::Handle_SetStart);
	PlayerInputComponent->BindAction(TEXT("SetEnd"), IE_Pressed, this, &AFGPlayer::Handle_SetEnd);
}
...
void AFGPlayer::Handle_SetEnd()
{
	FVector MouseWorldPos, TileWorldPos;
	GetMouseLocationOnGrid(MouseWorldPos);
	if(CurrentGridActor->TransformWorldLocationToTileLocation(MouseWorldPos, TileWorldPos))
	{
		BP_SetEnd(TileWorldPos);
	}
}

void AFGPlayer::Handle_SetStart()
{
	FVector MouseWorldPos, TileWorldPos;
	GetMouseLocationOnGrid(MouseWorldPos);
	if(CurrentGridActor->TransformWorldLocationToTileLocation(MouseWorldPos, TileWorldPos))
	{
		BP_SetStart(TileWorldPos);
	}
}
